...
| [`setup-bot.sh`](hermes/setup-bot.sh) | ボット用プロファイルをあなたのプロファイルの複製として作り（LLM はそのまま）、専用の API キーとポートを与え、両方を入れたうえで、全パターンを検査します |
...
新しいプロファイルは、Hermes 自身による現在のプロファイルの複製です（`hermes profile create --clone`）。そのためボットは、Grok でも OpenRouter でもローカルモデルでも、あなたの Hermes がすでに使っている LLM と同じ認証情報で動きます。キットがモデルを選ぶことはありません。Hermes はメッセージングのチャンネル設定を持ち込みませんが、そのプロファイルの `.env` の残りは引き継がれるので、ボットに持たせたくないものは削除してください。別のプロファイルを複製するなら `--clone-from <profile>` を使います。
...
ボットが扱うリポジトリに限定し、管理者権限のない fine-grained トークンを使ってください。フロアは分かりやすいミスを止めますが、何ができるかを決めるのはトークンです。`setup-bot.sh` は最後に、そのユーザーがどのアカウントでログインしているかを報告します。
